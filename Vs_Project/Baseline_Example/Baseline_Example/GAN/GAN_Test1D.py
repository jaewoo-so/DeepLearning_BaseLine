...
class GAN:
    ...
    def D_train_on_batch(self, Real, Gen):
        D = self.D
        print("D_train. Concatenate [Real,Gen_Fake]")
        X = np.concatenate([Real, Gen], axis=0)
        print("Current Y Data = [nb_Real : 1 + nb_Gen_Fake : 0]")
        y = np.array([1] * Real.shape[0] + [0] * Gen.shape[0])
        D.train_on_batch(X, y)

    def GD_train_on_batch(self, Z):
        GD = self.GD
        print("Current Y Data = [nb_Gen_Fake : 1] - Fake need to be output 1 ,if fake output is up to 0, G net will be updated ")
...
class Machine:
    ...
    def train_D(self):
        ...
        # Real data
        print("train_D Load Real Data Real")
        Real = data.real_sample(n_batch)  # (n_batch, ni_D)
        # print(Real.shape)
        # Generated data
        print("train_D Load Fake Data Z")
        Z = data.in_sample(n_batch)  # (n_batch, ni_D)

        print("Gen = G_Net ( Fake Data ) ")
        Gen = gan.G.predict(Z)  # (n_batch, ni_D)
        # print(Gen.shape)

        gan.D.trainable = True
        print("Start only D Train with[Real , Gen_Fake]")
    ...
    def train_GD(self):
        ...
        print("Start GD Train with[ Fake ]")
